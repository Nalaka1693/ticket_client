...
public class AvailabilityService {
	...
	public int processAvailabilityRequest(Request request) {
		...
		ServiceLogger.LOGGER.info("Availability request request validity cheking done");
		
		int totalPrice = 0;
		ArrayList<UnitInventory> unitInvs = this.reqProcessor.getUnitInventories(request);
		
		ServiceLogger.LOGGER.info("Availability request unit inventories creating done");
	...
	synchronized public int processReservationRequest(Request request) {
		...
		ServiceLogger.LOGGER.info("Reservation request unit inventories creating done");
	...
	public AvailabilityResponse createResponse(Request request, int totalPrice) {
		if (totalPrice > 0) {
			ServiceLogger.LOGGER.info("Availability true response creating...");
			return new AvailabilityResponse(request.getOriginCity(), request.getDestinationCity(),
					request.getNumOfTickets(), totalPrice);
		} else {
			ServiceLogger.LOGGER.info("Availability false response creating...");
